...
REF_GENOME=../../data/References/DM_1-3_516_R44_potato_genome_assembly.v6.1.fa
# Specifying the path containing the input data.
IN_PATH=../../data/GWAS_data/Final_BIG_GBS/trimmed_seqs
# Creaing and specifying the directory for the output alignment files
OUT_PATH_BAM=../../data/GWAS_data/Final_BIG_GBS/alignment_data
mkdir -p $OUT_PATH_BAM
# Creating and specifying the directory for the alignment statistics
OUT_PATH_STAT=../../analysis/alignment_data
...
for folder in $IN_PATH/*; do
	...
	bwa mem  -t $NTHREADS $REF_GENOME $fastq1 $fastq2 \
		|samtools view -@ $NTHREADS -ub - > $INIT_BAM #-ub: uncompressed, in BAM Format
	align_end=`date +%s`
	align_time=$((align_end-align_start))
	echo "bwa mem time in s:	$align_time" >> $COMP
	echo "Alignment and writing to BAM file done for sample ($count of $TOTAL)"
