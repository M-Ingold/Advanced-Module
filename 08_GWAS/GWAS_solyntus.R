...
genofile <- "/media/rna/CYSTOPHORA/GWAS/data/GWAS_data/Final_BIG_GBS/variant_calls_GWAS_using_solyntus/solyntus_filtered_GWASPoly.csv"
...
AddFrame <- cbind(dataAdd@map$Marker, "Chr09", dataAdd@map$Position, dataAdd@scores$Trait)
...
OneFrame <- cbind(dataOne@map$Marker, "Chr09", dataOne@map$Position, dataOne@scores$Trait)
...
GenFrame <- cbind(dataGen@map$Marker, "Chr09", dataGen@map$Position, dataGen@scores$Trait)
...
png(filename='manhattan_plot_solyntus.png', res=130, width = 1000, height = 700)
...
qqAdd <- qq.plot(dataAdd, trait = "Trait") + 
  ...
  annotate("text", label = "lambda[GC]^A == '1.08'", parse = TRUE, x = 0.7, y = 6, size = 4.5) + 
...
qqGen <- qq.plot(dataGen, trait = "Trait") + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        ...
        axis.title.y = element_text(size = 10),
        strip.text.x = element_blank()) + 
  annotate("text", label = "lambda[GC]^G == '0.99'", parse = TRUE, x = 0.7, y = 6, size = 4.5) + 
  geom_hline(aes(yintercept=4.5), color = "#1B9E77") + 
  xlim(0, 5) + 
  ylim(0, 6.5)

png(filename='qq_plots_solyntus.png', res=130, width = 1000, height = 1000)
# Defining layout for big plot
