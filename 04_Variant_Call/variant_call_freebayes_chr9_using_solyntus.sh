...
PATH_IN_BAMS=../../data/GWAS_data/Final_BIG_GBS/alignment_data_solyntus
PATH_BAM_LIST=bam_list_for_bamaddrg_solyntus.txt

# Input and output for Freebayes
PATH_REF_FILE=../../data/References/SolyntusV1.1.fasta
PATH_VCF_OUT=../../data/GWAS_data/Final_BIG_GBS/variant_calls_GWAS_using_solyntus
mkdir -p $PATH_VCF_OUT
PATH_STAT_OUT=../../analysis/variant_calls_freebayes_using_solyntus
...
VCF_OUT_FILE=$PATH_VCF_OUT/freebayes_${TOTAL_SAMPLES}_samples_09_solyntus.vcf
...
if [ -f "$BIG_BAM_FILE" ]; then
	...
	bdd_start=`date +%s`
	bamaddrg --clear --region chr09 $BAMADDRG_INPUT > $BIG_BAM_FILE
	bdd_end=`date +%s`
