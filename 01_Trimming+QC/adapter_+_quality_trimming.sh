...
ADAPTER_REF=/home/rna/BBTools/bbmap/BBMap_38.69/bbmap/resources/adapters.fa
PATH_READ_IN=../../data/GWAS_data/Final_BIG_GBS/RE_processed
PATH_READ_OUT=../../data/GWAS_data/Final_BIG_GBS/trimmed_seqs
PATH_STAT_OUT=../../analysis/GWAS_data/Final_BIG_GBS/trimmed_seqs
...
for folder in ${PATH_READ_IN}/*; do
	...
	R1=$folder/*R1*.bz2
        R1s=$(basename $R1)
        extension_part2=${R1s##*.} # extracting e.g. "bz2"
        filename1=${R1s%.*}
        extension_part1=${filename1##*.} # extracting e.g. "fastq"
        filename1=${filename1%.*}
        R2=$folder/*R2*.bz2
        R2s=$(basename $R2)
        filename2=${R2s%.*}
        filename2=${filename2%.*}
        extension=$extension_part1.$extension_part2 # creating e.g. "fastq.bz2"
